...
function draw() {
  ...
  if (gamestate === "end") {
    endpumpkin = createSprite(200, 200);
    endpumpkin.addImage("end", endpumpimg)
    textSize(20)
    fill("white");
    stroke("pink");
    strokeWeight(1)
    text("Uh-oh! The pumpkins have take over!", 400, 400);
    text("Don't worry, we can try again! Press R to play", 500, 500)
    luna.destroy();
   // bgsprite.destroy();
    ...
    if (mouseIsOver(endpumpkin)) {
      text("You cannot defeat me! MWHAHAHAHHA!", 300, 200)
    }
  }
  textSize(20);
  stroke("white")
